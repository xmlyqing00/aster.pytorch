...
def _normalize_text(text, lower_flag=True):
...
def get_str_list(output, target, dataset=None, lower_flag=True):
    # label_seq
    ...
    if True:
        ...
        pred_list = [_normalize_text(pred, lower_flag) for pred in pred_list]
        targ_list = [_normalize_text(targ, lower_flag) for targ in targ_list]
    else:
...
def Accuracy_with_lexicon(output, target, dataset=None, file_names=None):
    ...
    if len(file_names) == 0 or len(dataset.lexicons50[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(dataset.lexicons50[file_name], pred) for file_name, pred in
                             zip(file_names, pred_list)]
    ...
    if len(file_names) == 0 or len(dataset.lexicons1k[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(dataset.lexicons1k[file_name], pred) for file_name, pred in
                             zip(file_names, pred_list)]
        acc_list = [(pred == targ) for pred, targ in zip(refined_pred_list, targ_list)]
        accuracy = 1.0 * sum(acc_list) / len(acc_list)
        accuracys.append(accuracy)
    ...
    if len(file_names) == 0 or len(dataset.lexiconsfull[file_names[0]]) == 0:
        ...
        refined_pred_list = [_lexicon_search(dataset.lexiconsfull[file_name], pred) for file_name, pred in
                             zip(file_names, pred_list)]
        acc_list = [(pred == targ) for pred, targ in zip(refined_pred_list, targ_list)]
        accuracy = 1.0 * sum(acc_list) / len(acc_list)
        accuracys.append(accuracy)
